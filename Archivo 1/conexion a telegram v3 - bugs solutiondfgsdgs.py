...
def uv_i():
    try:
        ...
        sleep(2)  # Agrega un tiempo de espera antes de continuar
    except Exception as e:
...
def medir_sensores_humedad_suelo():
    try:
        ...
        if "ETIMEDOUT" in str(e):
            # Manejar el error de tiempo de espera (ETIMEDOUT)
            print("Error de tiempo de espera. Reintentando la medición de suelo.")
            sleep(2)  # Agrega una pausa antes de reintentar
            medir_sensores_humedad_suelo()  # Vuelve a llamar a la función
        else:
    ...
    try:
        ...
        while True:
            try:
                ...
                # Manejar cualquier excepción que ocurra durante la lectura de sensores
                print(f"Error al leer los sensores: {type(sensor_error).__name__}: {str(sensor_error)}")
                # Puedes agregar lógica adicional aquí según el tipo de excepción

            # Pausa entre iteraciones para evitar enviar mensajes continuamente
            sleep(300)  # Pausa de 5 minutos (en segundos)
